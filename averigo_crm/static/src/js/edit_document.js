...
export class EditPdfViewerField extends PdfViewerField {
    ...
    onLoadSuccess() {
        ...
        if (this.props.record.data.email_id || this.props.record.data.additional_emails) {
            ...
            if (this.props.record.data.additional_emails) {
                for (var i = 0; i < this.props.record.data.additional_emails.count; i++) {
                    email_count.push(this.props.record.data.additional_emails.records[i].data.display_name)
                }
            }
        ...
        } else if (this.props.record.data.email_ids) {
            ...
            $(this.iframePdf.el.contentWindow.document).find('#viewer').on('dblclick', function (e) {
                ...
                $(select).on('change', function (event) {
                    ...
                    var id = self.props.record.evalContextWithVirtualIds.active_id
                    self.orm.call("sow.action.wizard", "get_json_data", ["", self.tabs1, id], {})
                });
                $iFrame.contents().find("#viewer").append(select);
            })
        }
...
EditPdfViewerField.template = "EditPdfViewerField";
export const editPdfViewerField = {
    ...PdfViewerField,
    component: EditPdfViewerField,
}
registry.category("fields").add('pdf_viewer_edit', editPdfViewerField);
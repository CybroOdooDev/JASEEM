...
class CRMLead2OpportunityPartnerInherit(models.TransientModel):
    ...
    def create_customer(self):
        """ To create customer while converting lead to opportunity"""
        self.ensure_one()
        self.action = 'create'
        values = {
        ...
        for lead in leads:
            ...
            lead.convert_opportunity(lead.partner_id.id, user_ids=False,
                                     team_id=False)
        ...
        if user_ids:
            leads_to_allocate._handle_salesmen_assignment(user_ids,
    ...
    def action_set_won_rainbowman(self):
        """ to set customer context in partner while opportunity won"""
        if self.partner_id:
            self.partner_id.is_customer = True
        return super(CRMLead2OpportunityPartnerInherit, self).action_set_won_rainbowman()
